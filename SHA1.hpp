/*
    ...
    Updated beacuse of 3 year old c++ standards and compatibility issues
        -- Mohammad Jabar
*/
...
{
    return rol(static_cast<uint32_t>(block[(i + 13) & 15] ^ block[(i + 8) & 15] ^ block[(i + 2) & 15] ^ block[i]), static_cast<size_t>(1));
}
...
{
    z += ((w & (x ^ y)) ^ y) + block[i] + 0x5a827999 + rol(static_cast<uint32_t>(v), static_cast<size_t>(5));
    w = rol(static_cast<uint32_t>(w), static_cast<size_t>(30));
}
...
{
    block[i] = blk(block, i);
    z += (w ^ x ^ y) + block[i] + 0x6ed9eba1 + rol(static_cast<uint32_t>(v), static_cast<size_t>(5));
    w = rol(static_cast<uint32_t>(w), static_cast<size_t>(30));
}
...
{
    block[i] = blk(block, i);
    z += (((w | x) & y) | (w & x)) + block[i] + 0x8f1bbcdc + rol(static_cast<uint32_t>(v), static_cast<size_t>(5));
    w = rol(static_cast<uint32_t>(w), static_cast<size_t>(30));
}
...
{
    block[i] = blk(block, i);
    z += (w ^ x ^ y) + block[i] + 0xca62c1d6 + rol(static_cast<uint32_t>(v), static_cast<size_t>(5));
    w = rol(static_cast<uint32_t>(w), static_cast<size_t>(30));
}
